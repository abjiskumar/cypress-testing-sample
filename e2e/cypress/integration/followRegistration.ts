...
describe('Creating a new account by following an artist ', () => {
    ...
    it('Click on Browse tab, select Artists option and navigate to artists page', () => {
        cy.contains(btnText.home_browse).trigger('mouseover')
        cy.get(btnText.home_browse_artist).should('be.visible').click()
        cy.url().should('include', url.home_browse_artists)
    })
